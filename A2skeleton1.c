...
int fix_duplicate_list = 0; //this variable will ensure that my lists without duplicates will not be the same if several are generated.
void fill_without_duplicates(int *array, int size)
{
    
    srand( time(NULL));
          //+ fix_duplicate_list); //calling this initializes the random num generator each time
    ...
    for( int i = 0; i < size; i++){
        ...
        int swap_index = rand() % size + fix_duplicate_list; //for each element in the array, generating a number between 0 and size-1;
    ...
    fix_duplicate_list++;
    srand( time(NULL)); //initalise again for if function called twice
}
